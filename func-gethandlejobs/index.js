    ...
    for (const job in jobs.status) {
      if (Object.hasOwnProperty.call(jobs.status, job)) {
        ...
        if (status === 'failed') {
          ...
          logger.info('Updating the failedJobs array for {JobId}', jobId)
          try {
            ...
            for (const task of currentTasks) {
              ...
              if (task.status === 'waiting' || task.status === 'inprogress' || task.status === 'failed') {
                if (task.status === 'failed' && task.retry === 7) {
                  logger.info('The task with index: {CurrentTaskIndex} have failed 7 times. Whole job is set to failed', currentTaskIndex)
                  const filter = { _id: jobId }
                  const update = {
                ...
                try {
                  ...
                  try {
                    ...
                    logger.errorException(error, 'The job: {Job} with mongoDB id: {JobId} failed. Task with the index {CurrentTaskIndex} failed. Check the teams warning for more info!', job, jobId, currentJob)
                    const errorObj = {
